...
int main (int argc, char **argv) {
    ...
    //do each trial
    completeGraph *curGraph;
    int err;
    float curWeight;
    for(unsigned i = 0; i < numTrials; i++) {

        //Get new Graph and initialize it
        curGraph = genCompleteGraph(numPoints);
        if(!curGraph) {
            printf("Memory error: could not generate Graph on %dth trial.\n", i);
            free(treeWeights);
            return 1;
        }
        err = eucPopulateCompleteGraph(curGraph, dimension);
        if (err) {
            printf("Error: could not populate Graph on %dth trial.\n", i);
            destroyCompleteGraph(curGraph);
            free(treeWeights);
            return 1;
        }

        //Find the MST weight and clean up
        curWeight = findMST_Weight(curGraph);
        if (curWeight == 0) {
